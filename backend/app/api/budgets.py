...
def list_budgets(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return [BudgetOut(id=r.id, category=r.category, amount=r.amount, period=r.period, fund_source=r.fund_source, reference_date=r.reference_date) for r in rows]
...
def update_budget(budget_id: int, payload: BudgetUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return BudgetOut(id=r.id, category=r.category, amount=r.amount, period=r.period, fund_source=r.fund_source, reference_date=r.reference_date)
