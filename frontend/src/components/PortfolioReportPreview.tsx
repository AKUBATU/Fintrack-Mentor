  ...
  return <div className="portfolio-form-overlay financial-report-overlay fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="portfolio-report-title">
    <div className="financial-report-modal flex h-full w-full flex-col overflow-hidden bg-gray-100 sm:h-auto sm:max-h-[94dvh] sm:max-w-5xl sm:rounded-xl">
      <div className="financial-report-toolbar flex shrink-0 flex-col gap-3 border-b border-gray-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0"><h2 id="portfolio-report-title" className="font-semibold text-gray-900">Preview Laporan Portfolio</h2><p className="text-xs text-gray-500">Nilai aset merupakan nilai terkini; periode berlaku untuk aktivitas investasi.</p></div>
        <div className="financial-report-actions flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={() => window.print()} className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white"><Printer className="h-4 w-4" /> Simpan PDF</button>
      ...
      <div className="financial-report-scroll min-h-0 flex-1 overflow-y-auto p-3 sm:p-6"><article className="financial-report-document mx-auto w-full max-w-4xl overflow-hidden rounded-xl bg-white shadow-sm">
        ...
        <div className="p-5 sm:p-8">
          ...
          <div className="portfolio-report-activity mt-7 grid grid-cols-1 gap-5 sm:grid-cols-2"><section><h3 className="text-sm font-semibold text-gray-900">Transaksi saham</h3><p className="mb-3 text-xs text-gray-500">{activityLabel} · {filteredTransactions.length} transaksi</p><div className="space-y-2">{filteredTransactions.map((item) => <div key={item.id} className="flex justify-between gap-3 border-b border-gray-100 pb-2 text-xs"><div><p className="font-medium text-gray-900">{item.ticker} · {item.type}</p><p className="text-gray-500">{formatDate(item.date)} · {item.ownership}</p></div><p className="shrink-0 font-medium text-gray-900">{formatCurrency(item.total)}</p></div>)}{!filteredTransactions.length && <p className="text-xs text-gray-500">Tidak ada transaksi pada periode ini.</p>}</div></section>
