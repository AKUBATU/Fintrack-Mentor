...
interface FinancialReportPreviewProps {
  ...
  filteredPeriodLabel: string;
  fundSourceLabels: Record<string, string>;
  onClose: () => void;
  onDownloadCsv: (transactions: ExpenseTransaction[], scope: 'filtered' | 'all') => void;
}
...
export default function FinancialReportPreview({
  ...
  const transactions = scope === 'all' ? allTransactions : filteredTransactions;
  ...
  const reportPeriod = useMemo(() => {
    if (scope === 'filtered') return filteredPeriodLabel;
    if (transactions.length === 0) return 'Semua transaksi';
  ...
  }, [filteredPeriodLabel, scope, transactions]);
      ...
      <div className="financial-report-modal flex h-full w-full flex-col overflow-hidden bg-gray-100 sm:h-auto sm:max-h-[94dvh] sm:max-w-5xl sm:rounded-xl">
        <div className="financial-report-toolbar flex shrink-0 flex-col gap-3 border-b border-gray-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select value={scope} onChange={(event) => setScope(event.target.value as 'filtered' | 'all')} className="min-w-0 flex-1 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm sm:flex-none">
              <option value="filtered">Sesuai filter saat ini</option>
              <option value="all">Semua transaksi</option>
            </select>
            <button type="button" onClick={() => onDownloadCsv(transactions, scope)} disabled={transactions.length === 0} className="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-100 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 disabled:opacity-50">
