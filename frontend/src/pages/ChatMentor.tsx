...
export default function ChatMentor() {
  ...
  useEffect(() => setMessages((current) => current.length === 1 && current[0].id === 'welcome'
    ? [{ ...current[0], content: t('chat.welcome') }] : current), [language, t]);
  ...
  const handleSend = async () => {
    ...
    setLoading(true);
    try {
      const result = await api.chat(content, language);
      setMessages((current) => [...current, {
    ...
    <div className="chat-page flex flex-col">
      <div className="mentor-header mb-4">
        ...
        <div className="mentor-status-list">
          <span><ShieldCheck className="w-3.5 h-3.5" /> {t('chat.private')}</span>
          <span><CalendarClock className="w-3.5 h-3.5" /> {t('chat.daily')}</span>
        </div>
      </div>
      ...
      <div className="mentor-conversation bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
        <div className="mentor-conversation-bar">
          <div><p className="text-sm font-semibold text-gray-900">{t('chat.today')}</p><p className="text-xs text-gray-500">{t('chat.context')}</p></div>
          <span className="mentor-online"><span /> {t('chat.ready')}</span>
        </div>
        <div className="chat-messages overflow-y-auto p-6 space-y-4" aria-live="polite" aria-busy={loading || historyLoading}>
          ...
          {loading && (
            <div className="mentor-message flex gap-3">
              ...
              <div className="bg-gray-100 rounded-2xl px-4 py-3 flex items-center gap-2"><Loader className="w-4 h-4 animate-spin text-gray-600" /><span className="text-sm text-gray-600">{t('chat.thinking')}</span></div>
            </div>
          )}
        ...
        <div className="chat-input-area border-t border-gray-200 p-4">
          <div className="mentor-composer flex gap-2">
            <textarea rows={1} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={handleKeyDown} placeholder={t('chat.placeholder')} className="flex-1 resize-none px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent" disabled={loading || historyLoading} aria-label={t('chat.inputLabel')} />
            <button type="button" onClick={() => void handleSend()} disabled={loading || historyLoading || !input.trim()} className="mentor-send-button px-5 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" aria-label={t('chat.send')}>{loading || historyLoading ? <Loader className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}</button>
          </div>
          <p className="text-xs text-gray-500 mt-2 px-1">{t('chat.hint')}</p>
        </div>
      </div>
    </div>
  );
