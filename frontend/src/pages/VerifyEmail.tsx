...
export default function VerifyEmail() {
  ...
  const [message, setMessage] = useState(token ? t('verify.loading') : t('verify.invalid'));

  useEffect(() => {
    if (!token) return;
    api.verifyEmail(token)
      .then((result) => { setState('success'); setMessage(result.message); })
      .catch((error) => { setState('error'); setMessage(error instanceof Error ? error.message : t('verify.failed')); });
  }, [t, token]);
  ...
  return <main className="auth-shell min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
    ...
    <section className="auth-card max-w-md w-full bg-white rounded-2xl shadow-xl p-8 text-center">
      ...
      <h1 className="mt-5 text-2xl font-bold text-gray-900">{state === 'success' ? t('verify.successTitle') : state === 'loading' ? t('verify.loadingTitle') : t('verify.errorTitle')}</h1>
      <p className="mt-2 text-sm text-gray-600">{message}</p>
      <Link to="/login" className="mt-6 inline-flex rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-700">{t('verify.login')}</Link>
    </section>
  </main>;
}
